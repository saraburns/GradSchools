...
public class GradSchools {
  ...
  /*
   * methods added to improve the functionality of the GradSchools GUI
   */
  //getters:
  ...
  /*
   * deleteSchool()
   * used in the AddSchoolPanel ButtonListener class
   * given a String s,
   * deletes the School with name s from the School [].
   * @param s String  the name of the School to be deleted.
   */
  public void deleteSchool (String s) {
    //find the index of the School with name s
    int nameIndex = findName(s);
    //if the School with name s is in the School array,
    //remove the School
    if (nameIndex != -1) {
    //find the School to be removed
    School nameSchool = schools[nameIndex];
    //for each School after nameSchool,
    //shifts the School one to the left in the School []
    for (int i = nameIndex; i < numSchools-1; i++) {
    schools[i] = schools[i+1];
    }
    //decrement numSchools since one School was removed
    numSchools--;
    }
  }
   ...
   * findName()
   * used in the AddSchoolPanel ButtonListener class
   * given a String n, returns the index
   * of the first occurrence of the School with name n
   * in the schools array.
   * if there is no School with name n,
   * returns -1.
   * @param n String  name to check for
   * @return int  the index of the first School with name n
   */
  public int findName (String n) {
    for (int i = 0; i < numSchools; i++) {
      if (schools[i].getName().equals(n)) {
        return i;
      }
    }
    return -1;
  ...
  /*
   * getTopThree()
   * returns a School [] with the top three Schools.
   * this method is only called if the schools array has at least three Schools
   * so there is no need for an if statement or for any error Schools.
   * @return topThree  the array of the top three Schools
   */
  public School [] getTopThree() {
    //initialize the topThree array containing the top three Schools
    School [] topThree = new School [3];
    //if there are at least three Schools in the schools [],
    //set topThree to be the top three Schools
    //else fill topThree with error Schools
      topThree[0] = schools[numSchools-1]; //the top School
      topThree[1] = schools[numSchools-2]; //the second place School
      topThree[2] = schools[numSchools-3]; //the third place School
    return topThree;
  }
